...
For Google, the runner commits `remote_started` immediately before the one generation POST and commits the returned `interaction.id` separately as `provider_request_id` as soon as it can parse it. There is at most **one generation POST per logical attempt** and no automatic generation POST retry. Connection loss, timeout, HTTP 408, 429 and 5xx leave the outcome ambiguous; known ordinary 4xx are terminal. The eight known interaction statuses are `queued`, `in_progress`, `requires_action`, `completed`, `failed`, `cancelled`, `incomplete` and `budget_exceeded`. Pending statuses (`queued`, `in_progress`, `requires_action`) retain the interaction ID for GET-only resumption; known terminal failures (`failed`, `cancelled`, `incomplete`, `budget_exceeded`) retain the ID and reason without retrying generation. A completed result must have exactly one `audio/mpeg` audio output with valid base64 and decodable MP3. The adapter retains provider text outputs and any returned usage; actual billing cost remains unknown unless supplied by a trustworthy response.

`music-benchmark provider-resume --request-id ID` can perform a GET of the **stored** interaction ID to retrieve its status and, when complete, finish ingestion. Invoke it again if the same interaction remains pending; each invocation makes at most one GET. A returned `budget_exceeded` transitions the stored request to terminal failure. It never makes a generation POST. The provider-free `reconcile` command never contacts Google. Neither operation creates a fresh interaction to resolve an ambiguous POST whose ID was not returned.
