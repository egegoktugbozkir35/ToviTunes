...
The intended full pipeline is **creative planner → lyrics policy → music generation → music QA → timing → visual planning → animation → final QA → release policy**. The current `music_approval` gate approves only an audio candidate. Full episode release policy, real sung-word analysis, authenticated rights configuration, a real music adapter and visual/render integration remain future work.
