...
`AssetStore` ingests provider outputs and operator files through the same path. Call `Database.migrate()` first, then create an episode (or use an existing brand revision) before ingesting its files. Manual file paths are supplied by an operator. Provider and deterministic output paths must resolve under a configured `generated_source_roots` directory; without one, generated output ingestion is rejected.
...
`Provenance` records manual operator/source details or provider/model/request details. Ingestion appends explicit `unknown` rights and `pending` approval decisions. Later rights and approval decisions are separate append-only rows. An artifact can be selected only after current approval, file integrity and selected dependency checks pass. Unknown rights may permit intermediate selection, but a blocked rights decision prevents use. Publication will require explicit commercial clearance across all inputs in a later phase.
