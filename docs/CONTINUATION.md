...
After a selected TimedStoryboard, render and release planning holds at `character_pack` while any episode-pinned pack revision remains draft. This preserves the separate ability to plan and approve lyrics or music before original character art exists. The dry planner reads the pinned revision readiness; a future visual runner must also call `assess_pack_assets` against the matching approved manifest and recheck the episode lease before creating a scene.
