...
versioned brand + curriculum + CharacterAssetPack + policy
  ...
  → episode concept/story beats → educational/creative/safety review
  → lyrics → lyric review → EpisodeSpec + MusicSpec
...
| `EpisodeConcept`, `EpisodeSpec` | Objective, premise, cast, setting, vocabulary, story beats, reviewed lyrics snapshot/source ID and hash, desired structure, target duration, creative dimensions and pinned source versions. No exact scene timestamps: this reviewed pre-music contract is not the donor brief. |
...
* **E — lyrics rejected:** append the rejection with reviewer/reason; deselect that lyrics version. Preserve the objective, premise, and all old files for audit. Every selected artifact whose dependency chain includes the rejected lyrics (EpisodeSpec revision, MusicSpec/audio, TimedStoryboard, scenes that consume it, render, QA, metadata as applicable) becomes stale or unselected. Only artifacts with unchanged inputs can be explicitly rebound after validation and approval. A newly generated lyric version starts a new dependency branch.
