...
Status: proposal only, amended after the ToviTunes development plan. No production pipeline or provider integration is implemented by this document. Repository observations below describe the state verified on 2026-09-23 before the bootstrap commit and this draft PR. Donor source references are pinned to commit `09cc62d3e72918ad8b7aad47930390c880b09bd5` so later donor changes do not silently change the evidence.
...
Build one standalone local Python application, with no Boardroom Relics compatibility layer or generic multi-brand engine. The domain has `Brand → CharacterAssetPack/Curriculum → Episode → Artifact`; the first brand is ToviTunes, and additional characters are data. An episode pins the exact brand, character pack, curriculum, prompt, and policy versions it used. Default product values are English, ages about 3–6, 30–45 seconds, and YouTube Shorts, validated but configurable.

Use `src/tovitunes` to avoid accidentally importing from the working directory and to make tests exercise an installed package. Use Python `>=3.11`, `uv` for lockfile/development commands, Pydantic 2 at YAML/JSON/provider/database boundaries, and standard typed code inside the application. Use narrow interfaces for creative generation, music, images, optional video, deterministic character animation, rendering, and publishing, coordinated by a deterministic planner. SQLite owns durable state; files under a configured local data root hold large immutable artifacts. Logs describe execution but never decide whether work is complete. Generative AI may expand Tovi's world, but the versioned character pack controls Tovi's recurring identity.

An episode has one primary learning objective (a stable curriculum concept/version and an explicit outcome). A small traceability contract follows its objective ID and target vocabulary through `EpisodeSpec`, `LyricsSpec`, `MusicSpec`, `TimedStoryboard` scenes, visual specifications, render manifest, and QA findings. This permits a deterministic check that all stages claim the same target and a human or future visual check that the teaching imagery actually matches it. `EpisodeSpec` records the approved premise and story beats; `LyricsSpec` records separately reviewed lyrics before music generation. `TimedStoryboard` is derived only after an accepted audio master has been selected.

## E. Target pipeline

```text
versioned brand + curriculum + CharacterAssetPack + policy
  → select concept and pin learning objective → human objective review
  → EpisodeSpec concept/story beats → educational/creative/safety review
  → LyricsSpec → lyric review → MusicSpec
  → music candidate generation or import → objective audio QA
  → human song selection + rights review → APPROVED AUDIO MASTER
  → audio alignment + beat analysis → TimedStoryboard
  → scene/visual specifications → image assets/review + canonical Tovi animation
  → optional approved still → short VideoProvider clip/review
...
Each arrow consumes explicit accepted artifact versions. The planner can stop at any review or unmet requirement and later continue without replaying accepted expensive work. Lyrics and music are separate contracts. Objective audio checks reject unusable candidates; a human initially chooses the song for pronunciation, warmth, and musical quality. The selected song is the timeline's audio master. The `TimedStoryboard` must depend on its exact artifact ID/hash and alignment/beat analysis, and maps lyric words, scene bounds, gestures, animation cues, and learning purpose to the accepted recording. A changed audio master invalidates the timed storyboard and its timed descendants.
...
ToviTunes/
  ...
  brands/tovitunes/
    ...
    characters/tovi/packs/v1/     # approved pack manifest; canonical views and layers added when ready
  ...
  src/tovitunes/
    ...
    artifacts/                   # trusted paths, hashes, shared ingestion, validation
    pipeline/                    # requirements, pure planner, execution/lease coordination
    providers/                   # LLM, capability-aware music, image, optional video adapters
    animation/                   # CharacterAnimator; initial layered sprite implementation
...
| --- | --- |
| `BrandDefinition`, `BrandVersion`, `CharacterDefinition`, `CharacterAssetPack` | Stable brand/character IDs plus an independently versioned pack ID/hash. The pack contains approved front/three-quarter/profile references, exact palette, silhouette and proportions, eye/beak/tuft geometry, expressions, wing/gesture poses, mouth states, reusable sprite layers or rig data, allowed accessories, and forbidden visual changes. Episodes pin a pack version; no `if character == "Tovi"` paths. |
...
| `EpisodeConcept`, `EpisodeSpec` | Objective, premise, cast, setting, vocabulary, structured story beats, desired structure and pinned episode identity. Lyrics are a separate versioned artifact. No exact scene timestamps: this reviewed pre-music contract is not the donor brief. |
| `LyricsSpec`, `MusicSpec`, `MusicCapabilities`, `MusicCandidate` | Versioned lyrics and separate musical instructions: desired BPM/key/sections where supported, mood, instrumentation, voice/pronunciation, repetitions, call-response, duration, loopability and no-imitation constraints. Candidate records carry provider capabilities actually used, audio artifact ID, measured facts and QA findings. No unsupported control is silently assumed. |
| `TimedStoryboard`, `SceneSpec`, `VisualAssetSpec` | Derived from the accepted audio master plus alignment/beat artifacts. Ordered nonoverlapping scene IDs and exact time ranges, timed lyric/important-word cues, beat relationships, learning purpose, action/expression/gesture, setting/objects, target concept, composition, animation cues and required visual slots. Validate coverage of the actual song timeline. |
...
| `RenderManifest` | Exact scene/artifact and CharacterAssetPack versions/hashes, CharacterAnimator instructions, optional approved video clips, audio/SFX/caption inputs, frame/timebase, transforms, FFmpeg/tool versions and output settings; reproducible within a pinned toolchain. |
...
| `character_pack_revisions` | pack ID, character ID, independent semantic version, manifest hash, source revision, created_at | Unique character/version/hash; immutable pack identity. Its referenced binary assets use artifact IDs; visual-use approval is separate. |
| `episodes` | id, brand_revision_id, curriculum_concept_id/version, objective text/ID, language, target age/duration, lifecycle (`draft`, `active`, `held`, `complete`, `archived`), created/updated | Foreign keys to pinned versions; index brand, concept, created time. Lifecycle is not stage completion. |
| `episode_character_packs` | episode_id, character_id, character_pack_revision_id | Composite PK `(episode_id, character_id)` and FK to exact pack revision; pins every cast member, not just Tovi. |
...
| `generation_requests` | id, episode_id, stage/slot, provider/model/version, advertised capability snapshot and controls actually used, prompt/template hash and exact safe prompt location, input hashes, provider request ID, cost/idempotency key, status (`prepared`, `remote_started`, `succeeded`, `failed`, `ambiguous`), timestamps | Unique provider request ID when present; `(episode_id,stage,slot,status)`. Never silently repeat an ambiguous expensive request. |
...
An artifact is identified by an opaque ID and immutable bytes; a logical slot is `(episode, kind, scene/segment ID, variant)` or `(brand, reference role, character ID)`. The selected slot points to one version. Registration stores SHA-256, size, media type, trusted relative path, producer stage, exact provider/model/request ID, prompt and template version, acquisition/creation time, reference assets, and dependency IDs/hashes. Provider outputs and operator-supplied files pass through the same ingestion service, which probes media type, hashes bytes, records source/provenance and rights evidence, validates the file, assigns episode or brand ownership, and writes its approval state. A manual import is a producer with an operator/source URI and acquisition timestamp; it does not require a parallel `ManualMusicProvider` or `ManualImageProvider` hierarchy.
...
* **E — lyrics rejected:** append the rejection with reviewer/reason; deselect that lyrics version. Preserve the objective, EpisodeSpec premise/beats, and all old files for audit. Every selected artifact whose dependency chain includes the rejected lyrics (MusicSpec/audio, TimedStoryboard, scenes that consume it, render, QA, metadata as applicable) becomes stale or unselected. Only artifacts with unchanged inputs can be explicitly rebound after validation and approval. A newly generated lyric version starts a new dependency branch.
...
## K. Provider and animation boundaries
...
| `MusicProvider` | `capabilities() -> MusicCapabilities`, `generate(MusicSpec) -> GenerationJob`, `status(job) -> GenerationStatus`, `artifacts(job) -> list[MusicCandidate]`. Persist provider/model/request identity and capability snapshot. A candidate is ingested, objectively checked and then selected by a human. No scraping or consumer-session automation. |
| `ImageProvider` | Generates or edits backgrounds, props, environment/scene illustrations, transparent assets and occasional unusual Tovi poses from `VisualAssetSpec` and approved references. Output still passes ingestion and review; independent generation cannot redefine the canonical character pack. |
| `VideoProvider` | Optional approved-still-to-short-clip generation for a hero shot or unusual motion, normally a few seconds. Each clip is a separate scene dependency with QA/review; no episode or ordinary scene requires it. |
| `CharacterAnimator` | Deterministically animates a pinned `CharacterAssetPack` with timed gestures, expression, wing pose and mouth cues. Start with transparent layered sprites/simple skeletal transforms; `SpriteAnimator`, Spine or Live2D can implement the contract later without changing the storyboard or renderer. |
...
`MusicCapabilities` explicitly represents controls such as `exact_lyrics`, `word_timestamps`, `seed`, `bpm_control`, `key_control`, `negative_style`, `section_duration`, `instrumental_mode`, `editing`, and `stems`. The planner checks required capabilities and records which optional controls were actually used. If a provider lacks timestamps, local alignment is required after song selection; if it lacks exact-lyric control, lyric adherence QA must detect omissions or altered target words. Provider-specific controls may live in validated extension data rather than being silently discarded for a lowest-common-denominator API.

Providers and the animator do not write authoritative stage success directly. The application ingests and validates their outputs through the shared AssetStore; deterministic fakes implement each boundary for offline tests. Provider selection remains provisional until benchmark measurements of pronunciation, usable assets, rights, cost and latency support a choice.

## L. Brand and configuration

Version-controlled YAML defines the brand, curriculum and concept IDs (red, blue, yellow, green, orange, purple, pink, black, white, rainbow/review as initial data), creative bible, characters, safety policy and prompt templates. Each episode pins source Git revision, content hashes and a `CharacterAssetPack` version so editing `colors.v1.yaml` or releasing `tovi-pack-v2` affects only new episodes unless an explicit upgrade is chosen. A pack has a manifest with canonical views, palette, silhouette/proportion and facial geometry rules, expressions, gesture/wing poses, five initial mouth states (closed, small-open, wide/A, E/smile, O/round), reusable sprite layers or rig data, allowed accessories and prohibited changes. Its binary references are brand artifacts with IDs, provenance, rights and approval. Pack metadata can exist in Phase 1 before approved art is supplied; absence of art must block visual production, not episode identity. Add characters through data and generic IDs.
...
Educational checks compare concept/objective/vocabulary across EpisodeSpec, lyrics, MusicSpec and scenes; flag wrong concept, contradictions, weak repetition, age-inappropriate or unclear words, bad pronunciation guidance, and story overload. Music candidate QA checks duration, silence, clipping, loudness, vocal presence/entry, estimated BPM, lyric omissions and target-word occurrence. Those metrics reject objectively unusable candidates; initial selection of pronunciation, warmth and catchiness remains human. A scene teaching red with an orange teaching object is a representable `visual_concept_mismatch` issue tied to that scene and asset. Visual checks can progressively compare pack palette, silhouette, tuft, eye/wing anatomy, teaching-object color/count, scene continuity and safe areas. Multimodal semantic criticism is evidence for review, not mathematical proof; thresholds should come from accepted/rejected ToviTunes assets. Media QA is deterministic: trusted nonempty file and hash, expected MP4 container/video/audio codecs, 9:16 aspect and resolution, duration bounds, decodable streams, non-silent song, and clipping/loudness sanity. Only a current passing QA report against the exact final hash permits publication.

## P. Testing architecture

Default tests block network. Fakes return fixed typed LLM, capability-aware music, image, optional video and character-animation results with deterministic request IDs; a fake renderer writes small known fixtures, and a fake publisher models preflight failure, remote start, lost response and success. Use temp SQLite/data roots and a fake clock for leases. Test migration application/upgrade, model schema versions, pack-version pinning, shared manual/provider ingestion, capability fallback, song-selection-before-storyboard, audio-master replacement invalidation, path traversal and reparse/symlink escape, hash changes, atomic artifact registration, selected-version dependencies, scene-3-only resume, lyric rejection invalidation, rights unknown blocking publication, exact-MP4 retry, ambiguous upload hold/reconciliation, expected-channel mismatch, and Windows path behavior. Run a few local FFmpeg fixture checks only after the compositor PR; no live Suno, image model, Ollama, NVIDIA, OpenCode, Google or YouTube calls in CI. Match donor's Windows/offline CI pattern, adapted to `main`, the new package and test set.

## Q. Future implementation phases (small PRs)

1. **Foundation:** package/config/typed models, brand/curriculum/character-pack metadata loading, initial SQLite identities and migration tests (details below). No provider integration.
2. **Artifact store:** shared ingestion for generated and manual files, immutable registration, trusted-root/hash validation, provenance/rights/approval decisions and orphan recovery.
...
4. **Creative specifications:** objective/concept/story beats/lyrics/MusicSpec using a fake LLM and manual review records. Do not create timed scenes yet.
5. **Music benchmark:** compare provisional Suno, Eleven Music and Lyria 3 candidates using sensitive words such as `red`, `blue`, `three`, `circle` and `happy`. Measure target-word correctness/intelligibility, lyric adherence, objective audio quality, musical quality, rights, cost and latency before choosing an adapter. Capability-aware provider contracts and human song selection follow the evidence.
6. **Audio timing:** align the approved audio master and analyze beats; derive `TimedStoryboard` with exact lyric, gesture, educational and scene cues only after song selection.
7. **Canonical Tovi pack:** approve independent `tovi-pack-v1` references, palette, poses, mouth states, sprite layers and rights records; keep the pack version pinned per episode.
8. **Image benchmark:** use the same pack references and ten teaching/pose scenes across provisional Gemini 3.1 Flash Image and GPT Image 2.5 candidates; measure palette/silhouette/anatomy and educational correctness, and count usable assets per request and per cost. Add one adapter at a time.
9. **Character animation:** implement `SpriteAnimator` with a small idle, blink, point, flap, hop, emotion and entrance/exit library. Later rigs are optional implementations.
10. **Pilot compositor and QA:** compose approved stills, sprite motion and song locally with FFmpeg, run audio/visual/educational/media QA, and complete one reviewed 30–45 second Colors — Red pilot.
11. **Optional video:** add `VideoProvider` only for approved-still hero clips after the still/sprite pilot works; trim and validate clips locally.
12. **Publishing and later automation:** add official YouTube preflight/upload/reconciliation only after production is reliable. Use accepted/rejected episode data to reduce routine review and later evaluate retention; do not assume unattended publication at the pilot milestone.

## R. Recommended first coding PR

Keep the first implementation PR limited to **durable identity and typed boundaries**:

* Add `pyproject.toml`, `uv.lock`, `.gitignore`, `.github/workflows/ci.yml`, `src/tovitunes/__init__.py`, `src/tovitunes/config.py`, `src/tovitunes/domain/{brand,character,curriculum,episode,artifact,review}.py`, `src/tovitunes/persistence/{db.py,migrations/0001_initial.sql}`, `brands/tovitunes/{brand.yaml,creative_bible.yaml,safety_policy.yaml,curriculum/colors.v1.yaml,characters/tovi/character.yaml,characters/tovi/packs/v1/pack.yaml}`, `config.example.yaml`, `tests/{test_config,test_domain,test_migrations}.py`, and `docs/DEVELOPMENT.md`. Pack metadata can be versioned now; unapproved canonical image slots need no placeholder binary art.
* `0001_initial.sql` creates migration history, brand/character-pack revisions, episodes and their pinned character packs, artifact versions/selections/dependencies, and append-only rights/approval decisions with the identity/foreign-key constraints above. Later numbered migrations add request, run, lease, and publication tables immediately before the corresponding capability. Methods in this PR only create/read a pinned episode; artifact registration begins in PR 2, after file/hash validation exists.
* Acceptance: `uv sync --locked --extra dev`, Ruff, mypy and offline pytest pass on Windows; versioned YAML validates; an episode for `colors/red` pins brand, curriculum, objective and character-pack versions; migrated DB can reopen without drift; unknown rights and missing channel ID have no permissive defaults; path/slot IDs reject traversal; schema constraints prevent duplicate selected slots and orphan dependencies. Document exact local commands and ignored secret/data paths.
...
3. Which music provider earns selection after the pronunciation, capability, rights, cost and latency benchmark, and what commercial-use terms apply? No generated song should be marked rights-confirmed by default.
...
## T. First production milestone

The first production proof is one complete **30–45 second ToviTunes Colors — Red pilot**. It requires an approved objective, premise, lyrics and song; a `TimedStoryboard` tied to that song; an approved canonical Tovi pack; generated or imported background/props; deterministic character animation; a local 1080×1920 render; audio, visual, educational and media QA; full provenance/rights; and final human approval. Automatic YouTube publishing is outside this milestone. Success means a later episode can reuse Tovi's pack, preserve accepted expensive assets through failures, replace only stale scene slots, and reproduce which inputs and tools created the Short.
