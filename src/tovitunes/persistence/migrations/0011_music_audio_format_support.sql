-- Preserve the old WAV invariant while allowing validated original MP3 receipts.
...
WHEN NOT EXISTS (
    ...
    WHERE p.request_id = NEW.request_id
      ...
      AND ((p.mime_type = 'audio/wav' AND p.container = 'wav'
        ...
        OR (p.mime_type = 'audio/mpeg' AND p.container = 'mp3'
            AND p.codec = 'mp3' AND NEW.relative_path = NEW.request_id || '.mp3'))
)
...
WHEN NEW.status = 'succeeded' AND NOT EXISTS (
    ...
    WHERE p.request_id = NEW.request_id
      ...
      AND ((p.mime_type = 'audio/wav' AND p.container = 'wav'
        ...
        OR (p.mime_type = 'audio/mpeg' AND p.container = 'mp3'
            AND p.codec = 'mp3' AND o.relative_path = NEW.request_id || '.mp3'))
